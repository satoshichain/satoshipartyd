...
def validate (db, source, destination, asset, quantity, divisible, callable_, call_date, call_price, description, block_index):
    ...
    # Check for existence of fee funds.
        if quantity or (block_index >= 0 or config.TESTNET): # Protocol change.
           if not reissuance or (config.TESTNET): # Pay fee only upon first issuance. (Protocol change.)
...
def parse (db, tx, message):
    ...
    # Unpack message.
    try:
        if (tx['block_index'] > 0 or config.TESTNET) and len(message) == LENGTH_2: # Protocol change.
