...
def exact_penalty (db, address, block_index, order_match_id):
    ...
    if not (block_index >= 0 or config.TESTNET):   # Protocol change.
        # Order matches.
...
def cancel_order_match (db, order_match, status, block_index):
    ...
    if tx0_order['status'] in ('expired', 'cancelled'):
        ...
        if tx0_order['get_asset'] == config.BTC and (block_index >= 0 or config.TESTNET):    # Protocol change.
    ...
    if tx1_order['status'] in ('expired', 'cancelled'):
        ...
        if tx1_order['get_asset'] == config.BTC and (block_index >= 0 or config.TESTNET):    # Protocol change.
    ...
    if block_index < 0:    # Protocol change.
...
def match (db, tx, block_index=None):
    ...
    if tx['block_index'] > 0 or config.TESTNET:  # Protocol change.
    ...
    for tx0 in order_matches:
        ...
        if tx0_price <= tx1_inverse_price:
            ...
            fee = 0
            if block_index >= 0 or config.TESTNET: # Protocol change. Deduct fee_required from fee_provided_remaining, etc., if possible (else don’t match).
            ...
            # tx0
            tx0_status = 'open'
            if tx0_give_remaining <= 0 or (tx0_get_remaining <= 0 and (block_index >= 0 or config.TESTNET)):    # Protocol change
            ...
            #elif block_index >= 0 or config.TESTNET:    # Protocol change.
